...
Lectures needed for this project: Lecture 5 (Routing 2: Distance-Vector). Recommended: Lecture 4 (Routing 1: Principles).

You can work on this project alone, or with one partner. You can keep the same partner from Project 1, or choose a different partner. Reminder: [Our course policies page has a collaboration policy](/policies) that you must follow.
...
You can now access the visualizer at [http://127.0.0.1:4444](http://127.0.0.1:4444) in your browser. You should see the hosts and routers displayed against a purple background.
...
<table><thead>
  <tr>
    <th>Key<br></th>
    <th colspan="4">Value (TableEntry)</th>
  </tr></thead>
<tbody>
  <tr>
    <td>Destination</td>
...
To find out, start up the simulator and open [http://127.0.0.1:4444](http://127.0.0.1:4444) in your browser:
...
Next, let's implement Rule 6A (Split Horizon) of distance-vector.
...
Next, let's implement Rule 6B (Poison Reverse) of distance-vector.
...
Next, let's implement Rule 7 (Count to Infinity) of distance-vector.
...
Common bug: In Stage 2, don't forget to account for the fact that you shouldn't forward data packets if the distance in the table is `INFINITY`. If you forgot this, the tests won't catch your bug until Stage 8.
...
Next, let's implement Rule 5 (Poisoning Expired Routes) of distance-vector.
...
## Stage 10B: Triggered Updates on Link Up/Down
...
3. Finally, implement `handle_link_down`.
   ...
   If `self.POISON_ON_LINK_DOWN` is False, this function should delete all routes using that port.
...
Common bug: Make sure to use `force=False` to advertise incrementally when a link goes down.
...
To submit Project 2, submit only the `dv_router.py` file to the [Project 2 autograder on Gradescope]({{ site.data.projects.projects[2].gradescope}}).
